...
class StreamsIntroductionTests {
	...
	void forEachIfTest() {
		int[] ar = {1,10,1,1,2};
//		forEachIf(ar, n -> n%2 !=0, n -> System.out.println(n)); //- this each other
//		forEachIf(ar, n -> n%2 !=0, n -> System.out.println(n + " ")); //- this each other
		forEachIf(ar, n -> n%2 !=0, System.out::print); //- this each other  :: - method reference!!
	}
	@Test
	void getRandomArrayTest() {
		int[] ar = getRandomArray(10, 12, 1000);
		assertEquals(1000, ar.length);
		assertTrue(Arrays.stream(ar).allMatch(n -> n >=10 && n < 12)); // allMatch like as while
