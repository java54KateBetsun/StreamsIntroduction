...
public class StreamIntroductionMethods {
	...
	static public int maxIf(int ar[], Predicate<Integer> predicate) {
		return Arrays.stream(ar).filter(n -> predicate.test(n)).max().orElseThrow(() -> new NoSuchElementException("empty stream"));
	}
	...
	static public void displayShuffle(int ar[]) { // from interview
		//TODO
		//prints out a given array in shuffled
		//example, array: [10, 20, 30, 40], possible shuffling printing 30, 10, 40, 20
		//no additional arrays, no any collections
		//one code line
		new Random().ints(0, ar.length).distinct().mapToObj(i -> ar[i]).forEach(e -> System.out.print(e + " "));	
	}
}
